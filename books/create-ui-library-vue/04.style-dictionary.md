...
また、デザインシステムのトークンの定義はFigmaのようなデザインツール上で行われることが多いですが、実装はGitのようなバージョン管理システムを使うことが想定されます。デザインデータとソースコードの二重管理も避けたいところです。極論、デザインを変更したタイミングで実装の修正が自動で行われるべきでしょう。
