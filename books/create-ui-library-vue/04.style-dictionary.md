...
また、一元管理されたデザイントークンをGitHub Repositoryでコード管理することで、バージョニングやRelease Noteとの紐付け、GitHub Actions/Circle CIのような開発/デプロイ自動化ツールとの統合もやりやすくなります。
...
デザインシステムのアップデートは反復的に行われます。デザインが変更される度に実装を追従のは手間であり、自動化出来ないものかと考えることになるでしょう。
また、デザインシステムのトークンの定義はFigmaのようなデザインツール上で行われることが多いですが、実装はGitのようなバージョン管理システムを使うことが想定されます。デザインデータとソースコードとしてのデザイントークンの二重管理も避けたいところです。「デザインを変更したタイミングで実装の修正が自動で行われる」のが理想です。
